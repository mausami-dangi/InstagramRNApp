    ...
    renderNewsFeedFlatList = ({item}) => {
        return (
                ...
                <View style={{height: hp('50%'), backgroundColor: 'red'}}>
                    <View style={{flex: 0.13, backgroundColor: COLORS.BLACK_COLOR, alignItems: 'center', flexDirection: 'row'}}>
                        <Image style={styles.profileIcon}
                                source={{uri: item.image}}/>
                        <Text style={{color: COLORS.WHITE_COLOR, marginHorizontal: hp('1.5%')}}>{item.name}</Text>
                        <DotIcon name="dots-three-horizontal" color={COLORS.WHITE_COLOR} size={hp('2%')} style={{alignSelf: 'flex-end'}}/>
                    </View>
